...
*   [x] **Plan B (unblocks CI in the meantime):** `backend/main.py` now tries Bedrock first and, on any `boto3`/AWS error, automatically falls back to calling the same Claude model directly via the Anthropic API (`anthropic` SDK), authenticated with an `ANTHROPIC_API_KEY` secret (repo secret in CI, env var locally — never committed). This lets the pipeline produce a real tailored resume and move on to the monitoring phase without waiting on the Bedrock quota. See README's "Bedrock fallback" section.
...
*   [x] **Force-push of the rewritten history to `origin/main`** — confirmed done. `origin/main` was later re-verified: every historical blob (`git rev-list --objects --all`, scanned for email/phone-shaped strings) shows only `REDACTED_EMAIL`/`REDACTED_PHONE` or the harmless `jane.doe@example.com` placeholder from the very first commit — no real PII anywhere in history.
